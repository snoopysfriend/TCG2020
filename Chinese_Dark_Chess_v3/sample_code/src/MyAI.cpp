...
double MyAI::Nega_Scout(const int* board, int* move, const int red_chess_num, const int black_chess_num, const int* cover_chess, const int color, const int depth, const int remain_depth, double alpha, double beta){
	...
	if(move_count + flip_count == 0){ // terminal node (no move type)
		...
		double m = -DBL_MAX;
        double n = beta; // the current upper bound
		...
		for(int i = 0; i < move_count; i++){ // move
			...
			double t = -Nega_Scout(new_board, &new_move, new_red, new_black, new_cover, color^1, depth+1, remain_depth-1, -n, -max(alpha, m));
			if (t > m) { 
                if (n == beta || remain_depth < 3 || t > beta) { // the first leaf or the depth is too shallow (not good for scout)
                    m = t;
                } else {
			        m = -Nega_Scout(new_board, &new_move, new_red, new_black, new_cover, color^1, depth+1, remain_depth-1, -beta, -t); // re-search
                }
				*move = Moves[i];
			} else if (t == m) {
				bool r = rand()%2;
				if(r) *move = Moves[i];
		    } else if (m > beta) { // cut off 
                return m;
            }
            n = max(alpha, m) + 1; // set up a null window
		}
        // TODO the flip search
